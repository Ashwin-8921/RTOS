...
/* USER CODE BEGIN PV */
SemaphoreHandle_t xCountingSemaphore;  // counting semaphore handle
/* USER CODE END PV */
...
// Consumer Task → waits (takes) the semaphore
static void vConsumerTask(void *p)
{
    while (1)
    {
        ...
        {
            UART2_SendString("Consumer Task: Processing item\r\n");
        }
    }
}

// Producer Task → periodically gives (releases) semaphore tokens
static void vProducerTask(void *p)
{
    while (1)
    {
        ...
        UART2_SendString("Producer Task: Released 3 tokens\r\n");
