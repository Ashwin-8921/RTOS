...
{
	button_status ^= 1;  // toggle the value of button_status
	                     // (if 0 → 1, if 1 → 0)
}
...
{
  // Clear the interrupt flag for pin PC13
  HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_13);

  // Handle button press (toggle button_status)
  button_handler(NULL);
}

// Task to control LED depending on button status
void led_task(void *p)
{
	while(1)   // run forever
	{
		if(button_status == PRESSED)   // if button is pressed
		{
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, SET);   // LED ON
			vTaskDelay(1000);    // small delay
		}
